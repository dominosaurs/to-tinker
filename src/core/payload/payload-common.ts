export function buildSandboxPrelude(
    ...
    return [
        ...
        'register_shutdown_function(function () use (&$__toTinkerConnections, &$__toTinkerBaselineTransactionLevels) {',
        '    foreach ($__toTinkerConnections as $__toTinkerConnection) {',
        '        try {',
        '            $__toTinkerConnectionId = spl_object_hash($__toTinkerConnection);',
        '            $__toTinkerBaselineLevel = (int) ($__toTinkerBaselineTransactionLevels[$__toTinkerConnectionId] ?? 0);',
        '        } catch (Throwable $__toTinkerRollbackHashError) {',
        '            $__toTinkerBaselineLevel = 0;',
        '        }',
...
function buildTransactionPrelude(): string[] {
    return [
        ...
        '$__toTinkerBaselineTransactionLevels = [];',
        '$__toTinkerWrapConnection = static function ($__toTinkerConnection) use (&$__toTinkerConnections, &$__toTinkerWrappedConnectionIds, &$__toTinkerBaselineTransactionLevels): void {',
        ...
        '    }',
        '    try {',
        "        $__toTinkerCurrentTransactionLevel = method_exists($__toTinkerConnection, 'transactionLevel') ? (int) $__toTinkerConnection->transactionLevel() : 0;",
        '        if ($__toTinkerCurrentTransactionLevel > 0) {',
        '            $__toTinkerWrappedConnectionIds[$__toTinkerConnectionId] = true;',
        '            $__toTinkerBaselineTransactionLevels[$__toTinkerConnectionId] = $__toTinkerCurrentTransactionLevel;',
        ...
        '            $__toTinkerWrappedConnectionIds[$__toTinkerConnectionId] = true;',
        "            $__toTinkerBaselineTransactionLevels[$__toTinkerConnectionId] = method_exists($__toTinkerConnection, 'transactionLevel') ? max(0, (int) $__toTinkerConnection->transactionLevel() - 1) : 0;",
        '            $__toTinkerConnections[] = $__toTinkerConnection;',
        '        }',
