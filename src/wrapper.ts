...
export function buildTinkerPayload(options: WrapperOptions): string {
  ...
  return [...buildSandboxPrelude(options.sandboxEnabled, options.fakeStorage), options.selectionOrFileCode.trim(), ""].join(
    "\n",
  );
}
...
function buildSandboxPrelude(enabled: boolean, fakeStorage: boolean): string[] {
  ...
  return [
    ...
    ...(fakeStorage ? ["\\Illuminate\\Support\\Facades\\Storage::fake();"] : []),
