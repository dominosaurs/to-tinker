...
import { COMMANDS, RunKind } from "./commands";
import { getConfig } from "./config";
import { extractFile, extractSelection, findMethodAtPosition, MethodInfo } from "./extraction";
import { Output } from "./output";
import { promptForParameter } from "./php";
import { executeTinker, renderExecutionReport, RunRegistry } from "./runner";
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const register = (command: string, kind: RunKind, sandboxOverride?: boolean): void => {
    ...
    switch (kind) {
      case "selection":
        ...
        payload = buildTinkerPayload({
          ...
          selectionOrFileCode: extractSelection(document, editor.selection),
        });
        break;
      case "file":
        payload = buildTinkerPayload({
...
async function resolveMethodArguments(method: MethodInfo): Promise<Record<number, string>> {
  ...
  for (const [index, parameter] of method.parameters.entries()) {
    ...
    const value = await promptForParameter(parameter.name, parameter.signatureHint);
